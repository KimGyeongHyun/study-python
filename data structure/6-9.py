...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        if self.queue:
            pop_data = self.queue.popleft()
            print("{}님의 {} 문의 내용 접수 되었습니다. 담당자가 배정되면 {}로 연락드리겠습니다!".format(pop_data.name, pop_data.content,
                                                                           pop_data.email))
        else:
    ...
    def add_complaint(self, name, email, content):
        ...
        new_customer_complaint = CustomerComplaint(name, email, content)
        self.queue.append(new_customer_complaint)
