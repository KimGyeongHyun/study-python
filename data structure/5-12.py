...
class HashTable:
    ...
    def _find_linked_list(self, key):
        """해시 테이블에서 해당 인덱스의 링크드 리스트 리턴"""
        index = self._hash_function(key)
        return self._table[index]

    def _find_node(self, key):
        """해당 인덱스의 링크드 리스트에서 해당 key에 대한 노드 리턴"""
        find_linked_list = self._find_linked_list(key)
        return find_linked_list.find_node_with_key(key)
    ...
    def insert(self, key, value):
        ...
        if find_node is None:
            self._find_linked_list(key).append(key, value)
        else:
